...
| Files | `GET /servers/:id/files/list?path=` → `FileEntry[]` `{name, path, isDir, size, mode, modified}` (**field is `modified`, not `modifiedAt` — verified live**); `GET …/files/contents?path=` → envelope `data` is the **raw file text string** (NOT `{content}` — verified live); `POST …/files/write {path,content}`; rename `{from,to}`, delete `{paths:[]}`, mkdir `{path}`, compress `{paths:[],destination?}`, decompress `{path}`; `POST …/files/upload?path=<dir>` raw body **≤ 32 MiB hard cap** ("Use SFTP for larger files"); `GET …/files/download-url?path=` → **relative** signed URL `/servers/:id/files/download?path=…&exp=…&sig=…` (no `/api/v1`) — resolve as `apiOrigin + "/api/v1" + url` (Range-resumable, ~300 s TTL, `@Public` HMAC). **Ignore `POST …/files/upload-url`** — node-agent-internal path a client can't sign. |
